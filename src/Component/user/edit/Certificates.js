...
import { CustomInput, LabelText, Info } from "../../../controls/Input";
...
function Certificates(props) {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, action) => {
        if (awardFile === "") {
            return setFileMessage("Select atleast a file !!!");
          }      const data = {
        values,
        awardFile,
        userId,
      ...
      <Container>
        <Grid container style={{ textAlign: "center", marginBottom: 22 }}>
          ...
          <Grid item md={8}>
            ...
            {isError && (
              <MessageBox
                message="Your account Update has error"
                severity="error"
              />
            )}
            {isAwardEdited && (
              <MessageBox
                message={editedCertificates?.message}
                severity="success"
              />
            )}
          </Grid>
