...
export const GlobalProvider = ({ children }) => {
  ...
  const [loginpartnerState, loginpartnerDispatch] = useReducer(loginpartner, LoginInitialState)
  ...
  const [editAwardState, editAwardDispatch] = useReducer(editedFiles, ProfileInitialState)
  const [editStudentState, editStudentDispatch] = useReducer(editedStudent, ProfileInitialState)
  const [lectureState, lectureDispatch] = useReducer(lectures, LectureInitialState)
  const [topicState, topicDispatch] = useReducer(tables, LectureInitialState)
  const [deleteSubjectState, deleteSubjectDispatch] = useReducer(deleteSubject, LectureInitialState)
  const [deleteTopicState, deleteTopicDispatch] = useReducer(deleteTopic, LectureInitialState)
  const [editedTopicState, editedTopicDispatch] = useReducer(editedTopic, LectureInitialState)
  const [getlecturesState, getlecturesDispatch] = useReducer(getlectures, LectureInitialState)
  const [getAlectureState, getAlectureDispatch] = useReducer(getAlecture, LectureInitialState)
  const [editSubjectState, editSubjectDispatch] = useReducer(editSubject, LectureInitialState)
  const [enrollAlectureState, enrollAlectureDispatch] = useReducer(enrollAlecture, EnrolInitialState)
  const [myEnrolledlecturesState, myEnrolledlecturesDispatch] = useReducer(myEnrolledlectures, EnrolInitialState)
  const [timetableState, timetableDispatch] = useReducer(timetable, LectureInitialState)
  const [teacherLectureState, teacherLectureDispatch] = useReducer(teacherLectures, LectureInitialState)
